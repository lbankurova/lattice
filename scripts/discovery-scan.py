#!/usr/bin/env python3
"""
Discovery scan -- TEMPLATE (heavily pcc-driven, fork to your project).

Heuristic gap detection across a project's manifests, registries, and code.
Output is a ranked markdown report listing knowledge/architecture/coverage
gaps the agent could plausibly act on.
...
=== ADAPTATION REQUIRED FOR YOUR PROJECT ===

This file is the TEMPLATE. The reference implementation in
pcc/scripts/discovery-scan.py is heavily pcc-shaped: it assumes specific
manifests (capabilities.yaml, system-manifest.md, contract triangles,
methods-index, validation reference YAMLs, species-profiles). Your project
will not have these exact artifacts.

To use:
1. Copy to <your-project>/scripts/discovery-scan.py.
2. Edit the SCAN_CONFIG block: replace paths with your project's
   equivalent manifests, registries, and source directories. Drop entries
   that don't apply.
3. Per scan: read the function, decide whether the heuristic fits your
   conventions (e.g., scan_subsystems_missing_architecture assumes a
   markdown table with a Layer 1 column -- adapt to your manifest shape).
4. Drop scans that don't apply, add new scans for project-specific
   manifests. Keep the Gap dataclass + write_report() shape -- those are
   project-agnostic.
5. Run, iterate on signal quality. Target 80%+ real gaps.

Design rationale: see lattice/docs/literature/karpathy-llm-wiki.md
(gap detection as part of the lint operation) and ahrens-smart-notes.md
(emergence vs gap as dual signals).
...
# === EDIT THIS BLOCK ===
# All paths below are the pcc layout. Replace each with the equivalent in
# your project. Drop entries you don't have; the corresponding scan will
# fail and be reported as an error (other scans continue).
SCAN_CONFIG = {
    "capabilities":         ROOT / "docs/_internal/capabilities.yaml",         # capability/coverage manifest
    "system_manifest":      ROOT / "docs/_internal/knowledge/system-manifest.md",  # subsystem catalog with arch-doc column
    "architecture_dir":     ROOT / "docs/_internal/architecture",              # per-subsystem architecture docs
    "methods_index":        ROOT / "docs/_internal/knowledge/methods-index.md",   # catalog of scientific methods
    "analysis_dir":         ROOT / "backend/services/analysis",                # analysis-layer source code
    "validation_dir":       ROOT / "docs/validation/references",               # validation study YAMLs
    "species_profiles":     ROOT / "docs/_internal/knowledge/species-profiles.md", # species coverage doc
    "research_registry":    ROOT / "docs/_internal/research/REGISTRY.md",      # research streams registry
    "roadmap":              ROOT / "docs/_internal/ROADMAP.md",                # strategic roadmap
    "todo":                 ROOT / "docs/_internal/TODO.md",                   # tactical backlog
    "knowledge_dir":        ROOT / "docs/_internal/knowledge",                 # durable knowledge files
    "literature_dir":       ROOT / "docs/_internal/research/literature",       # literature note registry
}
# === END EDIT BLOCK ===
