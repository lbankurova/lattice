...
- [ ] **12. Contract triangle in sync (contract changes only).** If the commit modifies a contract field's declaration, enforcement, or consumption site (enum widened/narrowed, field added/renamed, nullability/cardinality changed), verify the OTHER TWO sites are updated in the SAME commit. Run the straggler grep: search the OLD vocabulary across `backend/tests/`, `docs/_internal/knowledge/*field-contracts*.md`, `docs/_internal/architecture/`, and frontend `types/` directories. Document the three sites in the commit body. Update `docs/_internal/knowledge/contract-triangles.md` if a new triangle was introduced or an existing one's site count grew. Skip only for pure UI/styling/test commits with no contract surface change. (See CLAUDE.md rule 18; exemplar: GAP-271 Phase 2 BFIELD-21.)
