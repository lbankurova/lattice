...
> **Rules:** 79 testable rules (C: 27, T: 9, S: 6, X: 7, K: 9, I: 11, A: 10) + 3 guiding principles.
...
| C-25 | **Domain labels: colored text only.** `getDomainBadgeColor(d).text` + `text-[10px] font-semibold`. Never dot badges, outline pills, bordered treatments. | Search for domain rendering. Any non-text treatment → FAIL. Hard rule. | Critical |
...
| I-11 | **Chart legends are interactive filters.** Every legend in a chart must toggle the visibility of the corresponding series/category on click. Legend items show a visual state change (e.g., muted text/struck-through/faded swatch) when their series is hidden. | Find any chart with a legend. Click a legend item → series doesn't toggle → FAIL. No visual feedback on toggled-off item → FAIL. | High |
...
| A-11 | **Spatial anchoring — paired displays share identical axes.** When two charts/tables share a common axis (dose groups, sex, timepoints), both must show the same categories in the same order, even if one panel has no data for some categories. Empty bar / "NE" indicator for missing data — never omit the row. Tab or mode switches must not cause categories to jump, reorder, or collapse. | Compare category axes across paired panels. Missing category in one panel → FAIL. Switch tabs/modes → axis categories move → FAIL. | Critical |
| A-11b | **Spatial anchoring — list indicator columns use fixed-width slots.** In scrollable lists with optional per-row indicators (badges, dots, text markers), each indicator type must have a fixed-width wrapper that always renders. Content fills the slot when present; the slot stays empty when absent. Indicators must align as scannable vertical columns during scroll. | Find conditional indicator rendering in list rows. Indicator uses `{cond && <span>}` without a fixed-width parent wrapper → FAIL. Scroll the list — indicators misalign across rows → FAIL. | High |
