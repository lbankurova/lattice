...
### FIX-02: R0 PreToolUse worktree-isolation hook activation tracking (5-observable gate)
- **Source:** /lattice:cycle GAP-357 build session 2026-05-16 — concurrent-staging conflation occurrence #5 surfaced during submodule commit (manual session vs concurrent `manual-pid-55386` running DATA-GAP-VEH-V2-INTACT-KIDNEY-P188). Two `git commit` attempts blocked at intent-mismatch in submodule pre-commit Step -0.5 before `safe-commit.sh` wrapper succeeded on third try.
- **Bug:** CLAUDE.md rule 21 specifies R0 PreToolUse hook (`hooks/preToolUse/require-worktree.sh`) is BUILT but NOT ACTIVATED, gated by 5 observables from R1 deployment (zero orphan worktrees > 24h, zero non-FF aborts, zero symlink failures or audited fallback, zero session-creation failures, hook block-event count > 0). The 5 observables are described in prose only — no tracking artifact (dashboard, telemetry file, weekly checklist) exists to drive the activation decision. Result: R1 (autopilot worktree beachhead) protects autonomous sessions, but manual sessions (which still run on canonical root) keep producing conflation events. The 4 → "3+ undercounted same-root-cause events after commit-intent deployment" → mine = ≥5 confirmed sweeps in pcc; detection alone has not prevented recurrence.
- **What needed:** a structured observable-tracking artifact in lattice (suggested: `docs/decisions/r0-activation-gate.md` — one row per observable, current value, threshold, last-checked date, action) wired into a recurring check (lattice:autopilot or scheduled review). When all 5 clear, R0 activates via `.claude/settings.json` PreToolUse hook entry.
- **Priority:** Medium — manual conflation events are detectable (intent-block at Step -0.5) but cost ~2-3 min per occurrence; activation will catch the class entirely.
