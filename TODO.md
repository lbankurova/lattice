...
### FIX-02: R0 PreToolUse worktree-isolation hook is REGISTERED but NOT FIRING on canonical-root commits
- **Source:** /lattice:cycle GAP-357 build session 2026-05-16 — concurrent-staging conflation occurrence #5 surfaced during submodule commit; later verification (2026-05-16 lattice cycle dispatch for original-FIX-02-framing) found R0 was actually active.
- **Bug:** R0 hook is REGISTERED in both `/c/pg/lattice/.claude/settings.json` and `/c/pg/pcc/.claude/settings.json` (PreToolUse matchers for `Bash(git add*|git commit*|git stash*)` and `Edit|Write` dispatching to `bash hooks/preToolUse/require-worktree.sh`). Activation timestamp: 2026-05-09 per `docs/decisions/worktree-isolation-2026-05-09.md:57` (commit `f37b8a8`). But **the hook is NOT firing on canonical-root git commits in practice.** Evidence: `/c/pg/pcc/.lattice/require-worktree-block.log` has only 2 entries from 2026-05-10T02:12 (the initial activation verification test); ZERO block events between 2026-05-10 and 2026-05-16 despite many canonical-root commits during that window (including 2+ from this session's GAP-357 build cycle, which committed directly from `/c/pg/pcc/` and from `/c/pg/pcc/docs/_internal/` without going through `lattice-session-start.sh`). The hook script itself (`hooks/preToolUse/require-worktree.sh`) inspected and logically sound — Bash-tool calls with no file_path fall through to Step 6 BLOCK after canonical-root detection at Step 2-3.
- **Doc drift:** lattice `CLAUDE.md` rule 21 still says R0 is "BUILT but NOT ACTIVATED" — contradicts the activation record in `docs/decisions/worktree-isolation-2026-05-09.md:57`. Same drift in `.lattice/worktree-isolation-protocol.md:3, 123`. Rule 21 text must be updated as part of this fix.
- **Candidate root causes (investigate before fixing):**
  1. **Matcher syntax** — `Bash(git add*|git commit*|git stash*)` may not parse correctly in Claude Code's hook matcher engine. The pipe-separated form may need different syntax (alternation regex, three separate matchers, etc.).
  2. **Session reload** — Claude Code may load `.claude/settings.json` only at session start; sessions started before 2026-05-09 won't see the hook. Need to verify whether the activation requires session restart or live-reloads.
  3. **docs/\* allowlist breadth** — `is_allowlisted` at line 165 permits all `docs/*` paths including `docs/_internal/*` (where the conflation surface lives). But Bash tool calls have no file_path → allowlist doesn't apply → should still block. Worth verifying interaction.
  4. **Silent early-exit path** — re-read the hook for any `exit 0` we missed (e.g., the submodule guard at line 81 sets `IN_CANONICAL=true` for submodules, which is the BLOCK direction not exempt — but the Bash-from-submodule flow needs trace).
- **What needed:** bugfix-cycle investigation that (a) reproduces the failure by running a controlled `git commit` from canonical root with the hook script invoked manually, (b) identifies the root cause from the 4 candidates above, (c) ships the fix (likely 1-line settings.json or hook-script change), (d) updates `CLAUDE.md` rule 21 + `.lattice/worktree-isolation-protocol.md` to reflect actual R0 state, (e) adds a regression check (block-log heartbeat: if N days pass with zero block events on canonical-root commits, alert).
- **Priority:** HIGH — R0 is the *intended* prevention layer for the concurrent-staging conflation class (5+ documented occurrences); without it firing, detection alone (commit-intent + Step -0.5 hook) keeps catching events at intent-mismatch but adds ~2-3 min friction per occurrence and depends on the user having declared correct intent. The protection layer is theoretically deployed but empirically non-functional.

### FIX-03: declare-commit-intent.sh / submodule pre-commit hook path-mismatch (pcc-side, cross-reference)
- **Source:** same session as FIX-02. NOT a lattice defect — `declare-commit-intent.sh` and `safe-commit.sh` are project-side per `CLAUDE.md:156` ("the framework's `mechanical-fix-cycle` invokes the script but does not ship it; consumers wire their own commit-intent protocol"). Filed here for cross-reference because the failure mode interacted with the lattice-shipped lock script during a lattice cycle.
- **Bug surface:** in pcc, `scripts/declare-commit-intent.sh` writes to `$REPO_ROOT/.lattice/commit-intent.txt` (line 23); pcc parent pre-commit hook reads `$(git rev-parse --git-path commit-intent.txt)` (per-worktree, `.git/commit-intent.txt`) at `.githooks/pre-commit:116`; pcc submodule pre-commit hook reads `$REPO_ROOT/.lattice/commit-intent.txt` (root-relative) at `docs/_internal/.githooks/pre-commit:110`. The two hooks check different files; the script writes to one location; per-worktree storage per CLAUDE.md rule 23 is documented in the script's own header comment but contradicted by its implementation. During GAP-357 commit, I had to manually `cp` between paths to get the parent hook to find the intent file. **Filing target:** pcc TODO.md (this is project-side wiring, not framework).
- **Companion issue:** lattice-shipped `safe-commit.sh` (also project-side per the same CLAUDE.md rule) is the wrapper that DOES work — it acquired the lock, declared intent, staged, and committed atomically on the third attempt, both in submodule and parent. Lesson is project-side: deprecate raw `git commit` from skills in favor of `safe-commit.sh`, OR fix the path-mismatch so raw `git commit` works.

### FIX-04: Cross-repo (parent / submodule) commit-lock invisibility (pcc-side, cross-reference)
- **Source:** same session as FIX-02. Parent-repo `.lattice/commit.lock/` and submodule `.lattice/commit.lock/` are separate directories — when parent's `acquire-lock.sh` holds the lock for a parent-scope operation, the submodule pre-commit Step -1 doesn't see it and waits / force-acquires as "stale" after 210s. The `LATTICE_LOCK_HOLDER` env-var bridge documented in `commands/lattice/review.md` Step 6 works for skill-internal commits but doesn't survive a `cd` into the submodule + raw `git commit`. **Filing target:** pcc TODO.md (cross-repo coordination is a project wiring concern; lattice's `acquire-lock.sh` does the right thing for a single repo).
- **Suggested fix (pcc-side):** submodule commits must always go through `safe-commit.sh` (verified working in this session); document this in CLAUDE.md submodule-commit guidance.

### FIX-05 (WTI-FU-2): Worktree submodule has a SEPARATE git dir cloned from origin -> stale; WTI-FU-1 `checkout main` does NOT fix it. Architect-gate premise invalidated.
- **Source:** pcc resume-work `worktree-build-tooling-fixes` 2026-05-31 (defect-3 investigation). Full record: pcc `.lattice/cycle-state/worktree-isolation.yaml` `post_ship_followup: WTI-FU-2` + `.lattice/decisions.log` 2026-05-31 `resume-work FINDINGS worktree-build-tooling-fixes`.
- **Bug:** Linked worktrees get their OWN per-worktree submodule git dir at `.git/worktrees/<wt>/modules/<name>`, NOT the shared canonical `.git/modules/<name>`. lattice-session-start.sh's `git submodule update --init` therefore clones the worktree submodule from the `.gitmodules` **origin URL**, which is behind canonical's unpushed LOCAL commits. WTI-FU-1's `git checkout <branch>` lands on the worktree submodule's OWN (origin-stale) branch tip, so it does NOT pull in canonical-local commits.
- **Premise invalidated:** the WTI-FU-1 architect-gate summary asserts "all superproject worktrees share one submodule git dir (`.git/modules/docs/_internal`)." This is FALSE for linked worktrees and was load-bearing for Candidate A's "shared serialized main." Requires an architect **re-gate**, not a hand-patch.
- **Evidence (pcc, 2026-05-31, worktree `…canonical-dataset-stage0…`):** canonical submodule main = `87e999c` (has BUG-048 via `e33cb69`); worktree submodule `git cat-file -t e33cb69` -> "Not a valid object name"; worktree submodule on main @ `7812f18` (origin-stale), committed BUG-SWEEP.md has 0 BUG-048. `.gitmodules` url = github origin.
- **Proposed fix direction (NOT implemented — needs re-gate + fresh-worktree reproduce-test):** after `submodule update --init`, fetch from canonical's LOCAL submodule store and land on canonical-local branch: `CANON_SM="$CANONICAL/.git/modules/<name>"; git -C "$WORKTREE/<sm_path>" fetch "$CANON_SM" "$sm_branch" && git -C "$WORKTREE/<sm_path>" checkout -B "$sm_branch" FETCH_HEAD`. Open Qs: interaction with the recorded superproject gitlink SHA; whether to push-to-origin instead (previously rejected — canonical-local is the coordination substrate); session-end merge-shared-state.sh interaction.
- **Target repo:** lattice (canonical; pcc mirrors via sync-skills). Lands in `scripts/lattice-session-start.sh` around the existing WTI-FU-1 block (~line 284-307).
...
### ~~ENH-17a: Integration phase (`lattice:integrate`) -- close the merge-back loop~~ -- DONE 2026-06-02
- **Source:** pcc resume-work `worktree-integration-automation` 2026-06-02 (handoff `.lattice/.continue-here-worktree-integration-automation.md`).
- **Problem:** Worktree isolation (R0/R1) shipped; integration did not. Manual worktree cycles (`build-cycle`/`review` run by a human) ended at `release-lock` and NEVER merged back -- 100% strand. Autopilot *did* call `lattice-session-end.sh --merge-back` on teardown, but that path is **fast-forward-only**: when the base advanced during the run (the normal case), the branch is *behind*, the FF aborts, and the worktree is left "for recovery" that nobody does. Cost was concrete -- BUG-049 was fixed on a stranded branch (`822bf52e`, 11 behind) then re-diagnosed and re-fixed on master (`626b48fe`): **two fixes for one bug.** The handoff's premise ("no skill has a merge step") was partly wrong (autopilot merges on the happy path); the real gap was the **rebase-when-behind + re-gate** fallback the script punted, plus zero merge-back on the manual path.
- **Resolution:** new `commands/lattice/integrate.md` skill (the "lander"): detect session-worktree context (no-op if not in one / if `LATTICE_AUTOPILOT_RUN=1`) -> verify review PASS + clean tree -> ahead/behind -> dispatch {remove dead | clean-FF | rebase+re-gate} -> `lattice-session-end.sh --merge-back`. `--sweep` drains all stranded session worktrees. Safety contract: **automatic merge only when green** (clean FF, or clean rebase that passes the re-gate); rebase conflict / red gate / dirty / unreviewed -> escalate, leave branch, NEVER force. Wiring: `scripts/lattice-session-end.sh` gains `--rebase` (rebase-when-behind, conflict->abort+exit2) + `--gate-cmd` (semantic re-gate; auto-discovered from `lattice-project.toml [project.integrate] gate_cmd`); `workflows/build-cycle.yaml` adds an `integrate` node after `spec-refresh`; `review.md` Step 7 invokes `/lattice:integrate`; `autopilot.md` + `autopilot.ts` teardown pass `--merge-back --rebase` and set `LATTICE_AUTOPILOT_RUN=1` so per-cycle integrate no-ops (autopilot owns batch teardown).

### ENH-17b (MISSED-GATE): review gate must verify the staged diff matches the spec's declared modified files, not just the working tree
- **Source:** user directive during ENH-17a (pcc resume-work `worktree-integration-automation`, 2026-06-02). Prevention class surfaced alongside the integration work.
- **Problem (the gate that's missing):** the review gate (`/lattice:review` + `scripts/write-review-gate.sh`) and the pre-commit staging-drift check (ENFORCEMENT.md Step 4) verify the *working tree* and the *staged set* against each other, and pcc's commit-intent protocol checks the staged set against a per-topic **declared file list**. But **nothing checks the staged diff against the SPEC's declared modified-files set** -- i.e. "did we change exactly the files the spec said this work would touch, no more, no less?" A spec can declare it modifies files {A, B}; the implementation can stage {A, C} (B silently dropped, C silently added) and the review still PASSes on a spec-vs-code trace of whatever *is* present. This is the same drift class as the stranded-work problem: work diverges from its declared intent and the gate doesn't see it because it only looks at the tree in front of it, not at the spec's contract.
- **Why it matters / failure mode:** (a) a spec requirement whose file was never touched (B dropped) traces as "not implemented" only if the reviewer happens to notice the absence -- there's no mechanical "spec said B, B not in diff" alarm; (b) scope creep (C added) lands unreviewed-against-spec; (c) combined with multi-session worktrees, a partially-implemented spec can merge looking complete. Spec-vs-code-of-what's-present is necessary but not sufficient; the gate must also assert diff-set == spec-declared-set (or surface every delta).
- **Shape of the fix (for the separate spec):** specs declare a machine-readable `modified_files:` / `touches:` set (frontmatter or a structured section). `write-review-gate.sh` (or a new review step) diffs `git diff --cached --name-only` against that set and reports MISSING (declared, not staged) + UNDECLARED (staged, not in spec) deltas; reviewer must account for each (legit scope change -> amend the spec's declared set in the same commit, mirroring commit-intent `--add`; accidental -> fix). Open design Qs: where the declared set lives (spec frontmatter vs `lattice-project.toml` vs cycle-state), interaction with commit-intent's per-topic file list (likely the spec set *is* the source for the intent declaration), tolerance for shared-state files (TODO/decisions.log/MANIFEST always-staged) -- probably an always-allow list mirroring the staging-drift exemptions.
- **Status:** filed as a missed-gate. **Worth a separate framework spec** (per user) -- not bundled into ENH-17a. Priority: Medium-High (it's a correctness gate on the spec->code contract, and it composes with the worktree-integration work just landed -- integration lands what review passed, so review needs to be sure what passed matches the spec).
