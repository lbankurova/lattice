...
- **VERIFY-DS-01: Hook DOES NOT fire on relative-path command (verified 2026-04-28). Re-verify after absolute-path fix.** Tested in pcc session 2026-04-28: `bash scripts/design-session.sh begin "verify"` set the lock to `preamble: pending`; subsequent `Edit` on `frontend/src/.../OnsetDoseDropdown.tsx` was NOT blocked (file was modified, no BLOCKED stderr surfaced). Direct script invocation (`CLAUDE_TOOL_INPUT='{"file_path":"...tsx"}' bash scripts/design-mode-gate.sh`) works correctly — exit 1, full BLOCKED message. **Diagnosis:** the project-level hook command in pcc `.claude/settings.json` is `bash scripts/design-mode-gate.sh` (relative path). Claude Code on Windows fires PreToolUse hooks but the relative path silently fails to resolve — the hook never executes. The user-level Write|Edit hook in `~/.claude/settings.json` on the same machine uses absolute MSYS path (`bash /c/Users/laris/.claude/hooks/...`) and works. **Lattice-side fix applied (this commit):** `scaffold/.claude/settings.json` now ships the design-mode hook with portable `bash -c 'ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"; bash "$ROOT/scripts/design-mode-gate.sh"'` pattern — git-rev-parse resolves repo root regardless of harness cwd. **pcc-side fix needed:** sendex `.claude/settings.json` PreToolUse Write|Edit block must be updated to either the absolute path or the git-rev-parse pattern. Filed as a sendex follow-up (not framework-side); sync-skills.sh does NOT touch project-level settings.json. **Re-verify protocol (next session):** (a) `bash scripts/design-session.sh begin "verify"`, (b) attempt Write/Edit on any `.tsx` file under `frontend/src/`, (c) confirm BLOCKED stderr surfaces and file is not modified, (d) `bash scripts/design-session.sh end`. Implementation references: lattice `de8c1af` (hook) + `09843ee` (cd-to-root), sendex `1a574964` (settings wiring), lattice `<TBD>` (this commit — scaffold hook with portable path). Delete this entry once re-verified.
