...
- **Blueprint complete →** print: `"Blueprint phase complete. Cycle state saved: phase=blueprint-complete. The build phase runs in its own git worktree — from the canonical tree run 'bash scripts/lattice-build.sh {topic}' (provisions a session/{topic} worktree and launches a fresh Claude session inside it), then '/lattice:cycle {topic}' to start the build. When the build cycle finishes, '/lattice:integrate' lands the branch and removes the worktree."` Log a `PHASE_TRANSITION_PENDING` row. STOP.
  - **Why build isolates and research/blueprint don't:** the build phase writes code (`backend/`, `frontend/`, `scripts/`) — the paths the R0 require-worktree hook blocks at canonical. Research and blueprint write only allowlisted paths (`docs/**`, `.lattice/**`, `.claude/**`) and run in canonical. A running session can't be relocated into a worktree (no hook/frontmatter does it; `EnterWorktree` is model-discretion only), so the only reliable isolation is to LAUNCH the build session in the worktree — which is what `lattice-build.sh` does. A build started in canonical instead fails safe: R0 blocks the first code write rather than conflating the index.
  - **Autopilot is exempt:** the `--continue` path (below) auto-dispatches build in-session under `LATTICE_AUTOPILOT_RUN=1`, where autopilot already owns one batch-scoped worktree. Do not use `lattice-build.sh` inside autopilot.
