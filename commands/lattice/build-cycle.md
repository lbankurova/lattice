...
**Per-phase, not just at entry (build-only / late-UI-phase guard).** The check above fires at build *entry* against the whole topic. That misses two cases: (a) a multi-phase build whose UI surface appears only in a **late phase** (entry runs while early phases are backend-only, so the surface is not yet in view); (b) a **`build-only` entry** that skipped the blueprint-time Section 1d gate entirely. In both, re-run this prerequisite at the **start of each UI-touching phase**, against *that phase's actual consumer surface*, before it implements — do not lean solely on the build-end Step 3 spec-refresh. A late phase that **re-sources an existing consumer** (port + retire in-memory compute, swap a data source, route a renderer to a backend artifact) must additionally verify **field-set sufficiency** — the new source carries *every field the consumer renders, at the right grain* — AND that the parity/regression oracle captures that **full consumed field set**, not a lossy subset (else "byte-equal to the oracle" is a tautological green that cannot detect a projection gap). Precedent: GAP-F4-FALLBACK-RETIRE Phase F — a backend verdict artifact that dropped two *rendered* sub-fields (`upgrade_evidence.items[]`, `discriminating_evidence.description/source`) and whose parity fixture was itself a lossy projection; the entry-time check passed (Phases A–E were backend) and the gap surfaced only at the Phase-F build read, after five days of parity work validated against a fixture that defined "correct" as "reproduces the projection."
