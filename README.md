...
**Mechanism — prevention layer (worktree isolation):**
- Worktree isolation per write-capable session (CLAUDE.md rule 21, `.lattice/worktree-isolation-protocol.md`). Git's index is shared per-repo; concurrent sessions in the same checkout race on a single staging area. Each session — manual, autopilot, MCP background, scheduled remote agent — runs in its own `git worktree` (own index, own branch `session/<topic>-<ts>`), spawned by `scripts/lattice-session-start.sh` and torn down via `scripts/lattice-session-end.sh --merge-back` (FF default; `--branch-as-pr` for multi-day refactors). The conflation class is structurally impossible: each batch's `git add` writes to its own index, never the canonical's.
- Active layers: R1 — autopilot beachhead (`executor/src/autopilot.ts` spawns/teardown by default; `LATTICE_AUTOPILOT_WORKTREE=0` opts out). R3 — `executor/src/e2e.ts` branch-mode uses detached worktrees instead of stash+checkout. R0 — project-wide `hooks/preToolUse/require-worktree.sh` PreToolUse hook (registered by `bash scripts/install-hooks.sh --enable-r0`) refuses Edit/Write/`Bash(git add|commit|stash)` at canonical root unless (a) Tier 1 allowlist (CLAUDE.md, README.md, ROADMAP.md, `.claude/`, `.lattice/`, `docs/`, plus per-project `[project.worktree] allow_main_tree_paths`), or (b) Tier 2 exemption envelope `LATTICE_ALLOW_MAIN_TREE=1 LATTICE_EXEMPTION_RATIONALE="<>=10 chars>"`. Cross-worktree state visibility for `.lattice/`: symlink primary, `LATTICE_PROJECT_ROOT` env-var fallback (Windows-without-Developer-Mode); 16 lattice scripts honor it. Crashed-session cleanup: `scripts/lattice-worktree-prune.sh` (interactive or auto-confirm-merged-only); chained from post-commit every N=10 commits.
- Borrowed prior art: `obra/superpowers/skills/using-git-worktrees/SKILL.md` (Step 0 detection-via-`GIT_DIR != GIT_COMMON_DIR`, submodule guard, `.worktrees/` directory convention, project-setup auto-detection); lattice extensions add enforcement, cross-worktree state, merge-back contract, exemption envelope, and crash recovery.

**Mechanism — detection layer (backstop, even within a worktree):**
...
- Parallel sessions conflated commits → prevention layer of per-session git worktrees (PreToolUse hook refuses canonical writes; `lattice-session-start.sh` spawns isolated index per session), with detection backstop of outer-held commit lock, per-topic WIP lock with re-entrancy and audited force-clears, revision-checked atomic state writes, staging-drift detection at hook exit.
