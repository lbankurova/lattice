...
19. **Atomic facts must live in the typed knowledge graph.** Any *atomic, contradictable* domain fact — numeric threshold, species-specific baseline, route/vehicle constraint, regulatory cutoff, mechanistic disable-marker — MUST live in the project's typed knowledge graph (`docs/_internal/knowledge/knowledge-graph.md` by default, per F1) as a typed YAML fact (with `value`, `confidence`, `scope`, `derives_from`, and `contradicts`). Un-typed registries (methods indexes, species/vehicle profiles, etc.) keep their indexing role and may *cite* a typed fact, but cannot be the authoritative home for a load-bearing numeric/categorical value. Why: only the typed graph audits contradictions mechanically (`audit-knowledge-graph.py` provenance-gap + within-graph-contradiction checks); the same claim authored as prose in two un-typed files can silently disagree. **Test:** if you find yourself writing a numeric threshold or species-specific value into an un-typed registry or a spec body, stop — promote it to the typed graph first, then cite the fact ID from the un-typed registry's narrative or the spec text. Exemption: descriptive narrative that contextualizes a typed fact (e.g., explaining why a threshold exists) belongs in prose; the *value* must be typed. Failure mode prevented: HCD-FACT-008 ↔ FACT-010 disagreement was caught only because both lived in the typed graph with `contradicts` edges; the same disagreement in prose form would have shipped. Exemplar: pcc CLAUDE.md rule 22 (2026-04-27) ported framework-side as LIT-01-FOLLOW-1b. Architect (`commands/lattice/architect.md` Step 1.4 spec lint) and peer-review (synthesis tier) ask the placement question on every algorithmic spec.

20. **Bug retrospective on every fix.** After every bug fix (`fix:` commit, or `feat:` commit with embedded behavioral correction): run `/ops:bug-stress` Step 8 (5-question retro). Output appended to the project's bug-log entry (path resolved via `[project.bugs] bug_log` in `lattice-project.toml`; SENDEX-shape default is `docs/_internal/BUG-SWEEP.md`). Five questions: (1) **Root cause** in one sentence; (2) **Genesis** — what decision/process produced it; (3) **Detection gap** — which gates did it pass through invisibly and why each missed it; (4) **Prevention class** — what process change would catch this *class* of bug; (5) **Lattice change** — concrete edit (rule N tightened / skill X gets step Y / hook Z added). The pre-commit hook BLOCKS `fix:` commits if the bug-log entry referenced by the commit lacks the 5 retro fields. Failure mode: bug recurs in same form because the gate that should have caught it was never tightened.

21. **Worktree isolation enforced at session entry (R0; R1+R2+R3 active 2026-05-09).** Git's index is shared per-repo, so concurrent sessions (autopilot vs manual, parallel autopilots, MCP background, scheduled remote agents, autonomous loops) can stage into a single index and a `git commit` can sweep a different session's work into the wrong commit. Documented incidents: 4 CONFLATED-COMMIT events (`1370c103`, `521f1d16`, `a47ee865`, `abdb31c9`) plus 3+ undercounted same-root-cause events (`45f29b53`, `c9f82aa`, `32944cf0`) -- the last two AFTER commit-intent deployment, confirming detection alone is insufficient. The prevention layer: every write-capable session runs in its own git worktree (separate index, separate branch). **Active layers:** R1 -- autopilot beachhead (`executor/src/autopilot.ts` defaults to spawning a session worktree via `scripts/lattice-session-start.sh` and tearing down via `scripts/lattice-session-end.sh`; opt out per-run with `LATTICE_AUTOPILOT_WORKTREE=0`); R2 -- review-agent isolation (`isolation: worktree` + read-only `tools` allowlist in `agents/{architect-reviewer,peer-review,decision-auditor,post-impl-reviewer}.md` frontmatter); R3 -- e2e branch worktrees (`executor/src/e2e.ts` uses detached worktrees instead of stash+checkout, leaving the user's tree untouched). **R0 (project-wide PreToolUse hook) ACTIVATED per project — lattice 2026-05-09 (commit `f37b8a8`), pcc 2026-05-16 (FIX-02 cycle).** Each project must commit its own `.claude/settings.json` R0 entries via `bash scripts/install-hooks.sh --enable-r0`; activation is per-`.claude/settings.json`, not framework-wide. **Matcher syntax requirement (FIX-02):** the R0 matcher MUST be 3 separate entries (`Bash(git add*)`, `Bash(git commit*)`, `Bash(git stash*)`) plus `Edit|Write` — pipe-alternation inside parens (`Bash(git add*|git commit*|...)`) is not reliably honored by Claude Code's matcher engine and silently fails to fire. Original R1-deployment activation gate (5 observables: zero orphan worktrees > 24h, zero non-FF aborts, zero symlink failures or audited fallback, zero session-creation failures, hook block-event count > 0) cleared 2026-05-09 for lattice; equivalent activation for pcc deferred until FIX-02 surfaced the pipe-alternation defect 2026-05-16. **Operationally:** the hook blocks Edit/Write/`Bash(git add|commit|stash)` at canonical root unless (a) Tier 1 allowlist hit (CLAUDE.md, README.md, ROADMAP.md, .claude/, .lattice/, docs/), (b) Tier 2 exemption envelope (`LATTICE_ALLOW_MAIN_TREE=1 LATTICE_EXEMPTION_RATIONALE="<>=10-char specific reason>"`). **Verification after activation:** `.lattice/require-worktree-block.log` should accumulate BLOCK entries on canonical-root sessions; persistent zero block count = matcher syntax or hook-script regression — investigate before assuming protection is live. Detection layer (commit-intent + acquire-lock + acquire-topic-lock) STAYS as backstop for intra-worktree conflation, semantic collision, and concurrent commits to shared `.lattice/` files. `.lattice/` cross-worktree visibility: symlink primary, `LATTICE_PROJECT_ROOT` env-var fallback on Windows-without-Developer-Mode (16 lattice scripts honor it). Merge-back semantics: `lattice-session-end.sh <topic> --merge-back` (FF default) or `--branch-as-pr` for multi-day refactors. Crashed-session cleanup: `lattice-worktree-prune.sh` (post-commit hook auto-confirm-merged-only; manual interactive). Full protocol: `.lattice/worktree-isolation-protocol.md`. Borrowed prior art: `obra/superpowers/skills/using-git-worktrees/SKILL.md` (detection-via-`GIT_DIR != GIT_COMMON_DIR`, submodule guard, `.worktrees/` convention, project-setup auto-detection); lattice extensions: enforcement at PreToolUse, cross-worktree state visibility, merge-back contract, exemption envelope, crash recovery.
