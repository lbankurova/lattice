...
- **Matcher syntax fix (FIX-02, applied to both projects 2026-05-16):** the original `Bash(git add*|git commit*|git stash*)` form (pipe-alternation inside parens) silently fails to fire on Claude Code's matcher engine. Empirically confirmed via `.lattice/require-worktree-block.log`: only 2 entries from 2026-05-10T02:12 (initial activation test) plus 0 events between 2026-05-10 and 2026-05-16 despite many canonical-root commits including a confirmed concurrent-staging conflation occurrence (#5) during pcc GAP-357 build cycle. The hook script itself works correctly when invoked directly — root cause was Claude Code's matcher not dispatching to it. Fix (attempted): split into 3 separate `Bash(...)` entries (`Bash(git add*)`, `Bash(git commit*)`, `Bash(git stash*)`) plus `Edit|Write`. **SUPERSEDED by FIX-03 (2026-05-29):** the split form ALSO never fired. Root cause was deeper than pipe-alternation -- `Bash(git add*)` is not valid *hook-matcher* syntax at all. Hook matchers match the tool NAME only (`Bash`, `Edit|Write`); the `ToolName(arg-pattern)` parenthesized form is PERMISSION-rule syntax and is read as a literal tool name that never matches. Only `Edit|Write` ever fired; the `Bash(git ...)` prong was inert from 2026-05-16 to 2026-05-29. FIX-03 replaces all three with a single bare `Bash` matcher and lets `require-worktree.sh` self-filter to git add/commit/stash (its Step 1).
...
**Current matcher form (FIX-03, both projects):** `.claude/settings.json` PreToolUse has TWO R0 matchers — `Bash` and `Edit|Write` — each dispatching to `bash hooks/preToolUse/require-worktree.sh`. The bare `Bash` matcher fires on every Bash call and the hook self-filters to git add/commit/stash internally (Step 1, matching the subcommand at a real command position -- command-value start or after a shell separator/space (; & | = or whitespace) -- so a prefix like `LATTICE_X=1 git commit` or `true; git commit` cannot bypass it, while a subcommand merely quoted in another command (`grep 'git add'`, `echo "git commit"`, `"Bash(git add*)"`) is NOT gated. Accepted residual: a subcommand in PROSE preceded by a space (`echo "note git commit"`) is regex-indistinguishable from `&& git commit` and still gates -- use the exemption). The R0 matchers MUST be ordered BEFORE the commit-lock matcher.

**Modes -- LENIENT (default) vs STRICT (FIX-03, mode gate added 2026-05-29):** R0 at canonical root has two modes, because a Claude session cannot relaunch itself into a worktree -- so an unconditional hard-block breaks interactive editing in a manually-launched canonical session. **LENIENT is the default:** Edit/Write and bash `git add/commit/stash` at canonical are PERMITTED (logged to `.lattice/r0-lenient-permit.log`); concurrency safety falls to the commit-level git hooks (commit-intent + commit-lock + review-gate), which is where the documented CONFLATED-COMMIT incidents actually occurred and are caught. **STRICT (`LATTICE_R0_STRICT=1`) hard-blocks** (exit 2), forcing work onto a worktree branch. **Best-of-both, by construction:** autopilot is already isolated via R1 (it runs in its own worktree, so it never reaches this gate at canonical) -- that is its 'strict'; a manual canonical session defaults to LENIENT so interactive work is unblocked; and a session that knowingly goes concurrent opts into STRICT. The liveness canary always probes the STRICT path (`LATTICE_R0_STRICT=1`) so it verifies the enforcement mechanism regardless of the session's mode. When STRICT, the escape hatches are: Escape hatches: (a) **inline exemption** parsed from the command string — `LATTICE_ALLOW_MAIN_TREE=1 LATTICE_EXEMPTION_RATIONALE="<reason >=10 chars>" git commit ...` (inline env vars do NOT reach the hook, so it parses them from `tool_input.command`); (b) **session env** `LATTICE_ALLOW_MAIN_TREE`/`LATTICE_EXEMPTION_RATIONALE` (e.g. via `CLAUDE_ENV_FILE`); (c) **`scripts/safe-commit.sh`** — its tool-command is `bash safe-commit.sh ...` with no `git commit` token, so the PreToolUse Bash hook does not gate it (the git pre-commit hook still runs). safe-commit is the sanctioned canonical-commit path; it commits to canonical HEAD, so it is a deliberate bypass, not full worktree isolation. **Known scope limit:** the submodule (`docs/_internal`) commits via safe-commit and so is NOT gated by this prong; its HEAD discipline is enforced by the submodule's own branch-guard (HEAD must be `main`), not by R0. **Submodule detached-HEAD fix (implemented):** a SessionStart hook (`scripts/submodule-checkout-main.sh`, matcher `startup|resume`) returns any detached submodule to `main` at session entry, so work and the branch-guard align from the start. `git checkout main` is the safety layer -- it carries non-conflicting uncommitted work onto `main` and refuses (with a warning) only on genuine conflict; no dirty pre-check, no auto-stash. Never blocks the session.
